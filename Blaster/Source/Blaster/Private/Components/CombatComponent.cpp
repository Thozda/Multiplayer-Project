...
void UCombatComponent::EquipWeapon(AWeapon* WeaponToEquip)
{
	...
	{
		AWeapon* OldPrimary = EquippedWeapon;
		EquipPrimaryWeapon(WeaponToEquip);
		EquipSecondaryWeapon(OldPrimary);
	}
	else
	{
		EquipPrimaryWeapon(WeaponToEquip);
...
void UCombatComponent::SetHUDCrosshairs(float DeltaTime)
{
	...
	if (Controller)
	{
		...
		if (HUD)
		{
			...
			if (CrosshairTargetFactorInterp != CrosshairTargetFactor)
			{
				CrosshairTargetFactorInterp = FMath::FInterpTo(CrosshairTargetFactorInterp, CrosshairTargetFactor,
					DeltaTime, 30.f);
			}
...
void UCombatComponent::ServerHitTarget_Implementation(const FVector_NetQuantize& Target)
{
	HitTarget = Target;
}
...
void UCombatComponent::StartFireTimer()
{
	...
	float FireDelay = EquippedWeapon ? EquippedWeapon->FireDelay : 0.1f;
