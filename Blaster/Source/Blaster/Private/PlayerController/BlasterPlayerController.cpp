	...
	if (CharacterOverlay == nullptr)
	{
		if (BlasterHUD && BlasterHUD->CharacterOverlay)
		{
			...
			if (CharacterOverlay)
			{
				...
				if (bInitializeBlueTeam) SetHUDBlueTeamScore(BlueTeamScore);
				if (bInitializePurpleTeam) SetHUDPurpleTeamScore(PurpleTeamScore);
...
FString ABlasterPlayerController::GetTeamsInfoText(ABlasterGameState* BlasterGameState)
{
	...
	FString InfoTextString;
	PurpleTeamScore = BlasterGameState->PurpleTeamScore;
	BlueTeamScore = BlasterGameState->BlueTeamScore;

	if (PurpleTeamScore == 0 && BlueTeamScore == 0)
	{
		InfoTextString = Announcement::ThereIsNoWinner;
	}
	else if (PurpleTeamScore == BlueTeamScore)
	{
		InfoTextString = FString::Printf(TEXT("%s\n"), *Announcement::TeamsTiedForTheWin);
		InfoTextString.Append(Announcement::PurpleTeam);
		InfoTextString.Append(FString("\n"));
	...
	{
		InfoTextString = Announcement::PurpleTeamWins;
		InfoTextString.Append(FString("\n"));
		InfoTextString.Append(FString::Printf(TEXT("%s: %d\n"), *Announcement::PurpleTeam, PurpleTeamScore));
		InfoTextString.Append(FString::Printf(TEXT("%s: %d\n"), *Announcement::BlueTeam, BlueTeamScore));
	}
	else if (BlueTeamScore > PurpleTeamScore)
	{
...
void ABlasterPlayerController::HideTeamScores()
{
	...
	bool bHUDValid = BlasterHUD &&
		BlasterHUD->CharacterOverlay &&
		BlasterHUD->CharacterOverlay->PurpleTeamScore &&
		BlasterHUD->CharacterOverlay->BlueTeamScore &&
		BlasterHUD->CharacterOverlay->ScoreSpacerText;
	if (bHUDValid)
	{
		BlasterHUD->CharacterOverlay->PurpleTeamScore->SetText(FText());
...
void ABlasterPlayerController::InitTeamScores()
{
	...
	if (bHUDValid)
	{
		FString Zero("0");
		FString Spacer("|");
		BlasterHUD->CharacterOverlay->PurpleTeamScore->SetText(FText::FromString(Zero));
		BlasterHUD->CharacterOverlay->BlueTeamScore->SetText(FText::FromString(Zero));
		BlasterHUD->CharacterOverlay->ScoreSpacerText->SetText(FText::FromString(Spacer));
	}
	else
	{
		bInitializeBlueTeam = true;
		bInitializePurpleTeam = true;
		BlueTeamScore = 0;
		PurpleTeamScore = 0;
	}
}

void ABlasterPlayerController::SetHUDPurpleTeamScore(int32 PurpleScore)
{
	BlasterHUD = BlasterHUD == nullptr ? Cast<ABlasterHUD>(GetHUD()) : BlasterHUD;
	...
	if (bHUDValid)
	{
		FString ScoreText = FString::Printf(TEXT("%d"), PurpleScore);
		BlasterHUD->CharacterOverlay->PurpleTeamScore->SetText(FText::FromString(ScoreText));
	}
}
